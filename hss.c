...
int hss_keygen(unsigned char *sk, unsigned char *pk) {
	...
	unsigned char aux[HSS_PRIVATE_KEY_aux];
	memset(aux, 0, HSS_PRIVATE_KEY_aux);
#else
	unsigned char *aux = calloc(HSS_PRIVATE_KEY_aux, sizeof(unsigned char));
#endif
	gen_hss_private_key(aux);
	gen_hss_public_key(aux, pk);
	memcpy(sk, aux, 64);
...
void recover_hss_private_key(unsigned char *sk) { //https://datatracker.ietf.org/doc/html/rfc8554#section-6.1
	/*sk->remain = (1 << (LEVELS * H));
	 sk->L = LEVELS;*/
...
int hss_sign(const unsigned char *message, const size_t input_size,
...
#endif
	memcpy(aux, sk, 64);
	recover_hss_private_key(aux);

	int to_refresh[LEVELS] = { 0 };
	int is_to_refresh = 0;
	unsigned long long L = bytes_to_ull(aux + 4, 4);
	for (unsigned int i = 1; i < L; i++) {
		if (is_exhausted(aux + 16 + (i * LMS_PRIV_KEY_SIZE))) {
			to_refresh[is_to_refresh] = i;
			is_to_refresh++;
		}
	}
	if (is_to_refresh != 0) {
		refresh_keys(aux, to_refresh, is_to_refresh);
	}
	...
	for (int i = 0; i < L - 1; i++) {
		...
		memcpy(signature + 4 + (i * (CRYPTO_BYTES_LMS)),
				aux + 8 + (L * LMS_PRIV_KEY_SIZE) + (L * LMS_PUB_KEY_SIZE)
						+ (i * CRYPTO_BYTES_LMS),
				CRYPTO_BYTES_LMS);
		...
		memcpy(
				signature + 4 + ((L - 1) * (CRYPTO_BYTES_LMS))
						+ (i * (LMS_PUB_KEY_SIZE)),
				aux + 8 + (L * LMS_PRIV_KEY_SIZE)
						+ ((i + 1) * LMS_PUB_KEY_SIZE),
				LMS_PUB_KEY_SIZE);

	}
	lms_sign_internal_f(message, input_size,
			aux + 8 + ((L - 1) * LMS_PRIV_KEY_SIZE),
			signature + 4 + ((L - 1) * (CRYPTO_BYTES_LMS))
